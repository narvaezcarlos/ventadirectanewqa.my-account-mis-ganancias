...
const Summary: React.FC<SummaryProps> = ({ userId }) => {
  ...
  if (!summary) {
    return (
      <div className={style.spinner__container}>
        <p className={style.spinner__container__text}>
          {" "}
    ...
    ); // Si isLoading es true, se muestra un componente de loading
  }
  ...
  const reportedDate = new Date(summary.updatedIn);
  ...
  return (
    <div className={style.summary__container}>
      ...
      <p className={style.summary__paragraph}>
        {" "}
        <span className={style.summary__span}>Mi link:</span>{" "}
        https://www.linkapp.com.co/tienda?id={link}
      </p>
      <p className={style.summary__paragraph}>
        Aquí te mostramos un resumen de como has crecido desde que estas con
        nosotros
      </p>
      <p className={style.summary__paragraph1}>
        <span className={style.summary__span}>Fecha de ingreso: </span>{" "}
        {createdDate.toLocaleDateString()}
      </p>
      <p className={style.summary__paragraph1}>
        <span className={style.summary__span}>Fecha de hoy: </span>{" "}
        {today.toLocaleDateString()}
      </p>
      <p className={style.summary__paragraph1}>
        <span className={style.summary__span}>
      ...
      <p className={style.summary__paragraph1}>
        {" "}
        <span className={style.summary__span}>Meses como linker: </span>
        {months}
        {months === 1 ? " mes" : " meses"}
      </p>
      <p className={style.summary__paragraph}>
